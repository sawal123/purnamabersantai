@php
    $heading = ($landingSectionHeadings ?? collect())->get('contact');
@endphp
...
<div class="relative overflow-x-hidden">
    ...
    @include('livewire.landing.partials.section-elements', ['pageSection' => 'contact', 'offset' => 0, 'variant' => 'page'])
            ...
            <div class="mx-auto max-w-6xl px-5 lg:px-8">
                <div class="reveal text-center">
                    <p class="landing-heading-kicker">
                        {{ $heading?->kicker ?: 'Official Contact' }}
                    </p>
                    <h1 class="mt-4 font-display text-5xl uppercase tracking-[0.08em] text-white sm:text-6xl">
                        @include('livewire.landing.partials.heading-title', ['heading' => $heading, 'fallbackTitle' => 'Direct Visitors to the Right Channel'])
                    </h1>
                    <p class="mx-auto mt-4 max-w-3xl text-lg leading-relaxed text-white/72">
                        {{ $heading?->subtitle ?: 'Pilih kanal resmi untuk ticketing, merchandise, partnership, atau update festival terbaru.' }}
                    </p>
                </div>
            </div>
        </section>
        ...
        <section class="relative z-10 pb-24">
            <div class="mx-auto grid max-w-6xl gap-6 px-5 md:grid-cols-2 xl:grid-cols-3 lg:px-8">
                @forelse ($contactChannels as $channel)
                    @php
                        $displayValue = $channel->value ?: $channel->url;
                    @endphp
